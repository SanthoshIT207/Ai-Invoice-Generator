...
if (error) setError("");
  ...
  // handle blur for basic validation
  const handleBlur = (e) => {
    const { name } = e.target;
  setTouched((prev) => ({
    ...prev,
  ...
  // valid when there are no error messages and both fields are non-empty
  return !emailError && !passwordError && !!formData.email && !!formData.password;
  };

  // submit handler — minimal, uses axiosInstance and API_PATHS
  const handleSubmit = async (e) => {
    const emailError = validateEmail(formData.email);
  const passwordError = validatePassword(formData.password);

  if (emailError || passwordError) {
    // show validation errors and mark fields as touched
    setFieldErrors({
      email: emailError,
  ...
  // clear UI state and indicate loading
  setIsLoading(true);
  setError("");
  setSuccess("");
  try {
