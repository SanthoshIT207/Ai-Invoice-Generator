...
const handleInputChange = (e, section, index) => {
  ...
  if (section) {
    ...
    // update top-level fields (notes, invoiceDate, paymentTerms, etc.)
    setFormData((prev) => ({ ...prev, [name]: value }));
  }
};
...
const handleAddItem = () => {
  setFormData({ 
    ...formData,
     items: [
      ...formData.items, { name: "", quantity: 1, unitPrice: 0, taxPercent: 0}] });
};

const handleRemoveItem = (index) => {

  const newItems = formData.items.filter((__dirname, i) => i !== index);
          ...
          <div className="grid gird-cols-1 md:grid-cols-3 gap-6"> 
            ...
            <div className="grid gird-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-white p-6 rounded-lg shadow-sm shadow-gray-100 border border-slate-200 space-y-4" >
                ...
                <InputField label="Email" type="email" value={formData.billFrom.email} onChange={(e) => handleInputChange(e, "billFrom")} />
              ...
              <div className="bg-white p-6 rounded-lg shadow-sm shadow-gray-100 border border-slate-200 space-y-4"> 
                ...
                <InputField label="Client Email" type="clientEmail" name="email" value={formData.billTo.email} onChange={(e) => handleInputChange(e, "billTo")} />
                                ...
                                <div className="space-y-4"> 
                                  <div className="flex justify-between text-sm text-slate-600"><p>Subtotal:</p><p>${subtotal.toFixed(2)}</p></div>
                                   <div className="flex justify-between text-sm text-slate-600"><p>Tax:</p><p>${taxTotal.toFixed(2)}</p></div>
                                    <div className="flex justify-between text-lg semibold text-slate-900 border-t border-slate-200 pt-4 mt-4"><p>Total:</p><p>${total.toFixed(2)}</p></div>
                                    </div>
                                    </div>
                                    </div>
